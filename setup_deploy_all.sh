#!/usr/bin/env bash
# FEDSTR All-in-One Setup
# Deploys customer + DVMs on the same machine.
# Good for local testing, paper reviewers, and single-machine demos.
# For separate customer/DVM deployment use setup_customer.sh / setup_dvm.sh.
...
print_header() {
    echo ""
    echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}${BOLD}║        FEDSTR All-in-One Setup                   ║${NC}"
    echo -e "${CYAN}${BOLD}║  Customer + DVMs on the same machine             ║${NC}"
    echo -e "${CYAN}${BOLD}╚══════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
echo "               Better for large models, fewer communication rounds needed."
echo ""
...
if [ "$ALGO_CHOICE" = "1" ]; then
    ...
    echo ""
    print_warn "DiLoCo: H=500 inner steps/round, 10+ outer rounds recommended."
    print_warn "Each round ~15-20 min on CPU, ~2-3 min on GPU (Lambda A10)."
fi

# ─────────────────────────────────────────────────────────────────────────────
print_step 5 "Federated learning parameters"

echo "  Configure the training run. Press Enter to keep defaults."
echo ""

NUM_DVMS=$(ask "Number of DVMs" "2")
ROUNDS=$(ask "Number of rounds" "$DEFAULT_ROUNDS")
EPOCHS=$(ask "Epochs per round" "$DEFAULT_EPOCHS")
...
echo ""
if [ "$STORAGE_CHOICE" = "1" ]; then
    echo "  This starts: local storage + ${NUM_DVMS} DVMs + customer [${ALGORITHM}]."
else
    echo "  This starts: ${NUM_DVMS} DVMs + customer (remote storage) [${ALGORITHM}]."
fi
